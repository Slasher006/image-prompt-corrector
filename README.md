...
Enable **Explicit adult (NSFW)** in the same location when adult content must be preserved or developed without euphemizing, censoring, or automatically adding clothing. Under creative rewrite settings, the adult contract develops sexual actions as well as visual detail: pacing, contact, physical response, facial expression, desire, pleasure, tension, tenderness, vulnerability, climax, and aftercare may be extended when compatible with the request. It is mutually exclusive with **Safe for work**, persists with settings, presets, and prompt history, and is used by correction, comics, memes, controlled variants, batches, and generated-image review. Requests containing underage or ambiguous-age terms are rejected before a model call; clearly adult wording such as `adult`, `age 24`, or `young adult` remains valid. Internal mode and age-guard instructions are not copied into the visible image prompt.
