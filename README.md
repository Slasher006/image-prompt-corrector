...
Enable **Explicit adult (NSFW)** in the same location when adult content must be preserved or developed without euphemizing, censoring, or automatically adding clothing. It is mutually exclusive with **Safe for work**, persists with settings, presets, and prompt history, and is used by correction, comics, memes, controlled variants, batches, and generated-image review. Requests containing underage or ambiguous-age terms are rejected before a model call; clearly adult wording such as `adult`, `age 24`, or `young adult` remains valid. Internal mode and age-guard instructions are not copied into the visible image prompt.
...
The draft and latest result are autosaved while you work and restored after the next launch. The counters below both editors show word totals and approximate token usage. Use the **Changes** tab to inspect additions, removals, and replacements. Output length is intentionally a qualitative preference so a useful prompt is not padded, cut, or repaired merely to satisfy a word count.
